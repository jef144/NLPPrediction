...
#Define the precition function
 predictWord <-  function(pre_grams) {
   ...
   if (nrow(dtPred) > 0) {
      dfResult <- rbind( dfResult, as.data.frame(cbind(head(dtPred[,post_gram, prob], 3), fallbackSeq), stringAsFactors=False))
   }   
 ...
 if (nrow(dfResult)  < 7 )   {
    #Try the 5gram Bag appraoch, need that all the words be found but ANY sequence is OK -- cf Bag of Words conccept
     ...
     fallbackSeq <- fallbackSeq + 1
     if (nrow(dtPred) > 0) {
       dfResult <- rbind( dfResult, as.data.table(cbind(head(dtPred[,post_gram, prob], 3), fallbackSeq)))
       dfResult <- dfResult[!duplicated(dfResult[,c('post_gram' )]),]
     }      
 } 
 ...
 if (nrow(dfResult)  < 7 )   {  
      ...
      fallbackSeq <- fallbackSeq + 1
      if (nrow(dtPred) > 0) {
        dfResult <- rbind( dfResult, as.data.frame(cbind(head(dtPred[,post_gram, prob], 3), fallbackSeq)))
        dfResult <- dfResult[!duplicated(dfResult[,c('post_gram' )]),]
      }   
 }
